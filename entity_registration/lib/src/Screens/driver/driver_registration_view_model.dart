...
// intialize
  ...
  Future<bool> registerDriver({
    ...
    int driverId = await _apiFunctionsService.registerDriver(
      userId,
      driverIssueCategory.text,
      type.name,
      driverTruckingCompanyId.text,
      idType.name,
      driverIdNumber.text,
      driverIdExpiryDate.toString(),
      driverIdIssueDate.toString(),
      country ?? "-1",
      driverLicenseType.text,
      driverLicenseNumber.text,
      licenseExpiryDate.toString(),
      state ?? "-1",
      country ?? "",
    );
    if (driverId == -1) {
      if (context.mounted) {
        Utils.showErrorToast(
            context: context, message: "Failed to register driver");
      }
      ...
      if (context.mounted) {
        Utils.showSuccessToast(
            context: context, message: "Driver Registered Successfully");
      }
      return true;
    }
  }
